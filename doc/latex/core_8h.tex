...
151         tbb::parallel\_for((\textcolor{keywordtype}{size\_t})0, polys.size(), (size\_t)1, \hyperlink{structSigSpatial2013_1_1pip}{pip}(tree, polys, results));
152 
153         \textcolor{keywordflow}{for} (\textcolor{keywordtype}{size\_t} i = 0; i < results.size(); ++i) \{
154             Result &r = results[i];
155             fo << r.first.first << \textcolor{charliteral}{':'} << r.first.second << \textcolor{charliteral}{':'}
156                << r.second.first << \textcolor{charliteral}{':'} << r.second.second << std::endl;
157         \}
158 
159     \}
160     fin\_point.close();
161     fo.close();
162 \}
\end{DoxyCode}
